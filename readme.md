...
- **It runs, but it has not been shown to match.** On a real 521d, SIGFM enrols and stores a proper print, and then fails to recognise the enrolled finger, for the reproducibility reason described above. Three crashes had to be fixed in the port to get that far (a missing GTask source tag, a double free of the keypoint data, and the enroll print being hardcoded to NBIS), so it is at least no longer a segfault — but it is not a working matcher on this sensor yet.
