...
Five patches are applied to that fork before building:
...
4. The sensor has a 1-pixel dead border (columns 0/63, rows 0/79) that reads ~600 against a ~4000 interior and is identical in every capture, so it survives into every image as a maximum-contrast frame that both matchers latch onto. The script extends the interior over it.
5. Its 52XD driver sets the match threshold to 24, well under libfprint's own default of 40, so fingers that were never enrolled could clear it. The script raises it to 40. `GOODIX_BZ3_THRESHOLD=<n>` builds a different value. See the warning below.
...
**This has now been measured on a real 521d, and it is worse than a threshold problem.** With patches 3 and 4 in place the reader enrols cleanly and `fprintd-verify` returns `verify-match` with a score of 74 against a threshold of 40 — the `No minutiae found` messages are gone, so the image genuinely improved. But a finger that was never enrolled scored **91 on the same build**: higher than the real one. No threshold can order those correctly.

Digging into the driver's own captured frames explains why. Two presses of the *same* finger produce images that share almost nothing: matching an image against itself agrees on 190–340 keypoints, matching two presses of one finger agrees on 0–26. The sensor's idle frame — the one used as the calibration reference — is saturated at the top of its 12-bit range, so subtracting it removes a constant offset but not the sensor's gain pattern, which only appears once a finger pulls the reading into the linear range.

So the images carry very little reproducible ridge detail, and both matchers are largely comparing noise. **Do not use this reader for login.** Test your own before trusting it:
...
- **Enrolling used to fail outright with `Not enough keypoints found`.** SIFT needs detail to key on, and before patch 3 there was none. With background subtraction and border repair the count is 180–340, well over the 25 minimum, and enrolment completes. If you still hit this, the image is the problem — lowering `GOODIX_SIGFM_MIN_KEYPOINTS` past it just stores a print that will match anybody.
- **It runs and it matches, but it still cannot be trusted.** Three crashes had to be fixed in the port to get this far (a missing GTask source tag, a double free of the keypoint data, and the enroll print being hardcoded to NBIS). With those fixed, plus background subtraction, border repair and 4-frame averaging, a real 521d enrols and recognises the enrolled finger. Over 18 measured attempts, though, genuine scores ran 37–607 and an unenrolled finger reached 838 — overlapping ranges, so **no threshold separates them**. (That 838 was a different finger that slipped during the press; five clean presses of an unenrolled thumb topped out at 88.) At the old default of 40 an unenrolled finger got in 38% of the time; the default is now 250, which fails closed instead. Measure your own with `debug.sh --match-test`.
